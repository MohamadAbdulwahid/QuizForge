...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/database/repositories/question.repository.ts` created with the following methods:
...
- [ ] `createMany` and `reorder` use `db.transaction()` for atomicity
- [ ] JSDoc comments on every public method
- [ ] Type-safe using `QUESTION` and `insertQuestion` types from `schema/quiz.ts`
- [ ] Exported as named functions (functional style)
- [ ] No compilation errors, passes linting
...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/api/dtos/quiz.dto.ts` created with the following Zod schemas:
...
- [ ] Conditional validation: `QuestionSchema` uses `.superRefine()` to enforce that `options` array is present and has ≥2 items when `type === 'multiple-choice'`, and `correct_answer` matches one of the option ids
- [ ] `apps/backend/src/api/dtos/session.dto.ts` created with:
  - `CreateSessionRequestSchema`:
    - `quiz_id`: `z.coerce.number().int().positive()`
  - `export type CreateSessionRequest = z.infer<typeof CreateSessionRequestSchema>`
- [ ] `apps/backend/src/api/middleware/validation.ts` created with:
...
- [ ] All schemas have descriptive Zod error messages (`.min(1, 'Title is required')`)
- [ ] No compilation errors, passes linting
...
**Definition of Done (DoD):**
- [ ] Share code generation utility created in `apps/backend/src/shared/utils/share-code.ts`:
...
- [ ] `generateUniqueShareCode()` function in `apps/backend/src/shared/utils/share-code.ts`:
...
- [ ] Integration with `quiz.repository.ts`: the `create()` method accepts a `shareCode` parameter (generated externally by the service layer, not internally by the repo)
- [ ] Unit tests in `apps/backend/tests/unit/shared/share-code.spec.ts`:
...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/api/services/quiz.service.ts` created with:
...
- [ ] Custom error classes created in `apps/backend/src/shared/errors.ts`:
...
- [ ] Error handler middleware (`error-handler.ts`) updated to handle custom error classes and map their `statusCode` and `code` to the JSON response
- [ ] Service uses logger: `logger.child({ component: 'quiz-service' })` for structured logging
...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/api/controllers/quiz.controller.ts` created with:
...
- [ ] `apps/backend/src/api/routes/quiz.routes.ts` created with:
...
- [ ] `apps/backend/src/api/routes/index.ts` created (or updated):
...
- [ ] Express app (`main.ts` or `app.ts`) updated to:
...
- [ ] Tspec annotations added to controller methods (JSDoc `@tag`, `@summary`, `@post`, `@patch`, `@security`)
- [ ] No compilation errors, passes linting
...
**Definition of Done (DoD):**
- [ ] `getMyQuizzes` method added to `quiz.controller.ts`:
  - Extracts `userId` from `req.user.sub`
...
- [ ] Response includes computed `questionCount` field (derived in service layer by counting questions array length, or via a SQL `COUNT()` subquery in the repository)
- [ ] Quizzes returned in descending `created_at` order (newest first)
- [ ] Manual testing:
...
**Definition of Done (DoD):**
- [ ] All quiz routes (`/api/quizzes/*`) require valid JWT:
...
- [ ] Public route exception defined:
...
- [ ] Verify `req.user` is available in all quiz controller methods (TypeScript type from Sprint 1's `AuthenticatedRequest`)
- [ ] Integration tests in `apps/backend/tests/integration/quiz-auth.spec.ts`:
...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/database/repositories/session.repository.ts` created with the following methods:
...
- [ ] PIN generation utility created in `apps/backend/src/shared/utils/pin.ts`:
...
- [ ] Unit tests in `apps/backend/tests/unit/shared/pin.spec.ts`:
...
- [ ] Type-safe using `Session`, `InsertSession`, `SessionStatus` types from `schema/session.ts`
- [ ] Exported as named functions (functional style)
- [ ] No compilation errors, passes linting
...
**Definition of Done (DoD):**
- [ ] `apps/backend/src/api/services/session.service.ts` created with:
...
- [ ] `apps/backend/src/api/controllers/session.controller.ts` created with:
...
- [ ] `apps/backend/src/api/routes/session.routes.ts` created with:
...
- [ ] Session router mounted in `routes/index.ts` at `/sessions`
- [ ] Manual testing:
...
- [ ] Test: `create()` inserts a quiz and returns it with an id
- [ ] Test: `findById()` returns quiz when it exists
- [ ] Test: `findById()` returns `null` for non-existent id
- [ ] Test: `findByIdWithQuestions()` returns quiz with questions in correct order
- [ ] Test: `findByCreator()` returns only quizzes by the given creator
- [ ] Test: `findByCreator()` returns empty array for creator with no quizzes
- [ ] Test: `findByShareCode()` returns quiz when code matches
- [ ] Test: `update()` modifies title and returns updated record
- [ ] Test: `update()` returns `null` for non-existent quiz
- [ ] Test: `delete()` returns `true` when quiz is deleted
- [ ] Test: `delete()` returns `false` for non-existent id
- [ ] Test: `shareCodeExists()` returns `true` for existing code, `false` otherwise

**Test File:** `apps/backend/tests/unit/repositories/question.repository.spec.ts`
- [ ] Test: `createMany()` inserts questions with correct `order_index` values
- [ ] Test: `createMany()` sets `quiz_id` on all questions
- [ ] Test: `findByQuizId()` returns questions ordered by `order_index`
- [ ] Test: `update()` modifies question fields
- [ ] Test: `delete()` removes a single question
- [ ] Test: `deleteByQuizId()` removes all questions for a quiz
- [ ] Test: `reorder()` updates `order_index` correctly in a transaction

**Test File:** `apps/backend/tests/unit/repositories/session.repository.spec.ts`
- [ ] Test: `create()` inserts a session and returns it
- [ ] Test: `findByPin()` returns session for active PIN
- [ ] Test: `findByPin()` excludes ended sessions
- [ ] Test: `updateStatus()` changes session status
- [ ] Test: `pinExists()` returns `true` for active sessions only

**Test File:** `apps/backend/tests/unit/shared/share-code.spec.ts`
- [ ] Test: Generated share codes are 8 characters
- [ ] Test: Codes use only allowed characters (no ambiguous chars)
- [ ] Test: 100 generated codes are all unique
- [ ] Test: Custom length parameter produces correct-length codes

**Test File:** `apps/backend/tests/unit/shared/pin.spec.ts`
- [ ] Test: Generated PINs are 6 digits
- [ ] Test: PINs are numeric only
- [ ] Test: PINs are zero-padded
- [ ] Test: 100 generated PINs are all unique

**Test File:** `apps/backend/tests/unit/services/quiz.service.spec.ts`
- [ ] Test: `createQuiz()` generates share code and inserts quiz + questions
- [ ] Test: `updateQuiz()` throws `ForbiddenError` if user doesn't own quiz
- [ ] Test: `updateQuiz()` throws `NotFoundError` for non-existent quiz
- [ ] Test: `updateQuiz()` replaces questions when `questions` array is provided
- [ ] Test: `deleteQuiz()` throws `ForbiddenError` if user doesn't own quiz
- [ ] Test: `getQuizByShareCode()` strips `correct_answer` from response

**Test File:** `apps/backend/tests/unit/dtos/quiz.dto.spec.ts`
- [ ] Test: Valid `CreateQuizRequest` passes validation
- [ ] Test: Missing title fails validation
- [ ] Test: Empty questions array fails validation
- [ ] Test: `multiple-choice` question without options fails validation
- [ ] Test: `correct_answer` not matching any option id fails validation
- [ ] Test: `time_limit` outside range (5-120) fails validation
- [ ] Test: Valid `UpdateQuizRequest` with partial fields passes
- [ ] Test: `QuizIdParamSchema` rejects non-numeric values

### Integration Tests

**Test File:** `apps/backend/tests/integration/quiz-endpoints.spec.ts`
- [ ] Test: `POST /api/quizzes` with valid data → 201, returns quiz with share code
- [ ] Test: `POST /api/quizzes` with invalid body → 400
- [ ] Test: `GET /api/quizzes` returns authenticated user's quizzes
- [ ] Test: `GET /api/quizzes/:id` returns quiz with questions
- [ ] Test: `PATCH /api/quizzes/:id` updates quiz fields
- [ ] Test: `DELETE /api/quizzes/:id` removes quiz → 204
- [ ] Test: `GET /api/quizzes/share/:code` returns quiz without correct answers (public)

**Test File:** `apps/backend/tests/integration/quiz-auth.spec.ts`
- [ ] Test: All quiz endpoints return 401 without token
- [ ] Test: Share code endpoint is publicly accessible
- [ ] Test: Users cannot modify/delete quizzes they don't own → 403

**Test File:** `apps/backend/tests/integration/session-endpoints.spec.ts`
- [ ] Test: `POST /api/sessions` creates session with unique PIN → 201
- [ ] Test: `POST /api/sessions` with non-existent quiz → 404
- [ ] Test: `POST /api/sessions` for quiz not owned by user → 403
- [ ] Test: Duplicate session for same quiz → 409

### Manual Testing Checklist

- [ ] Create quiz with Postman/curl → verify in Supabase dashboard
- [ ] Verify share code is 8 chars, uppercase, no ambiguous characters
- [ ] Create multiple quizzes → list them → verify order is newest first
- [ ] Update quiz title → verify in Supabase dashboard
- [ ] Delete quiz → verify questions also deleted (cascade)
- [ ] Create session → verify 6-digit PIN in response
- [ ] Attempt to create session for same quiz twice → expect 409
- [ ] Access quiz via share code without auth → expect public response (no answers)
- [ ] Verify all error responses follow format: `{ error, code, statusCode }`
- [ ] Verify structured logs appear with correct component tags

---

## Sprint 2 Success Metrics

- [ ] All 11 user stories completed (DoD met), including added PB-17.5
- [ ] 100% of unit tests passing
- [ ] 100% of integration tests passing
- [ ] Code coverage ≥80% for repository, service, and controller modules
